 ...
 * Run with: RUN_MANUAL_E2E=1 npx playwright test e2e/full-import.spec.ts --headed --timeout=600000
 */

test.describe("Full Import Flow", () => {
  test.skip(
    process.env.RUN_MANUAL_E2E !== "1",
    "Manual MSAL import flow is opt-in; set RUN_MANUAL_E2E=1 to run it."
  );
  ...
  test("complete hydration wizard and import", async ({ page }) => {
    ...
    await page.goto("/");
    await expect(page.getByText("v2.6")).toBeVisible();
