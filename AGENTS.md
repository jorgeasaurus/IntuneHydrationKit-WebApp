...
- Grow the system in layers. Start with the smallest complete product that works end to end. Add each capability only on top of a working product. Do not trade a working product for unfinished complexity.
- Keep components modular and keep concerns clearly separated.
- Prefer established, well-maintained libraries when they reduce complexity or improve reliability. Use existing project dependencies before writing custom code or adding packages. Check library documentation and types before you decide that a capability is absent.
- Make architectural decisions for the long term. Do not use stopgaps that are intended for replacement.
- Always use ASD-STE100 Simplified Technical English in documentation, comments, user-facing text, and agent responses.
- Pull requests must include a website version increment in `package.json` and `package-lock.json` before merge.
...
- Keep shared types in their Graph domain modules. Do not add TypeScript mirrors for policy templates in `public/IntuneTemplates`. Other template types can use TypeScript when needed.
